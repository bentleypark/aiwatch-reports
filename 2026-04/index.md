...
- **OpenAI was effectively the lone Major-LLM in good standing**: April's three Major-LLM general-purpose APIs ranked OpenAI (Score 84 / 97.44% uptime / Good), Claude API (Score 61 / Fair, 96.46% uptime, 40 per-model-inflated incidents), Gemini API (Score 62 / Fair, 80.15% estimated uptime, dragged down by the 242h key issue). Two of three Major-LLM providers landing in Fair tier in the same month is unusual — single-vendor LLM stacks on Anthropic or Google saw measurably worse April reliability than those on OpenAI. Strong argument for keeping at least a fallback layer in production.
...
- **OpenAI가 4월 Major-LLM 중 거의 유일하게 무난**: 4월 메이저 LLM 3종은 OpenAI(점수 84, 업타임 97.44%, Good 등급), Claude API(점수 61, Fair, 업타임 96.46%, 40건 — 모델별 집계로 부풀림), Gemini API(점수 62, Fair, 추정 업타임 80.15%, 242시간 API 키 장애 영향) 순이었습니다. 세 메이저 LLM 중 두 곳이 같은 달 Fair 등급으로 마친 건 드문 일로, Anthropic이나 Google에 단일 의존하는 LLM 스택은 OpenAI 쪽보다 측정 가능한 수준으로 신뢰성이 낮았습니다. 프로덕션에서는 최소한 fallback 레이어를 두는 게 합리적이라는 강력한 근거.
